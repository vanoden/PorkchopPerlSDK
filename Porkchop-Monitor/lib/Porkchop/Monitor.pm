#######################################################
### Porkchop::Monitor								###
### Interfaces with Porkchop::Monitor module API.	###
...
# This allows declaration	use Porkchop::Monitor ':all';
...
sub new
{
	...
	$self->{config} = {
		'protocol'		=> 'http',
		'path'			=> '',
		'user-agent'	=> 'Porkchop::Monitor/'.$VERSION,
	};
...
# Get a Collection Sensor
sub getCollectionSensor
{
	my $self = shift;
	my $parameters = shift;
	$self->{error} = undef;
...
# Add a Collection Sensor
sub addCollectionSensor
{
	my $self = shift;
	my $parameters = shift;
	$self->{error} = undef;
